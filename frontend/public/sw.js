/* CMMS-TPT Service Worker — PWA full-app shell v9
 * ยทบาย:
 ...
 *
 *  เมื่อ deploy UI ใหม่ ให้ bump SW_VERSION (v38 → v39 ...) หนึ่งบรรทัดเดียว:
...
self.addEventListener("fetch", (event) => {
  ...
  // 4) อื่น ๆ (ไฟล์ static ทั่วไป) — network-first + cache fallback
  event.respondWith(
    fetch(request)
      .then((response) => {
