...
try {
    ...
    switch ($method) {
        case 'GET':
            ...
            $keys = ['line_notify_enabled','line_notify_token','line_channel_access_token','line_channel_secret','line_channel_id','line_liff_id','line_callback_url','line_maintenance_group_id','line_group_enabled','low_stock_alert','maintenance_alert_days','email_notify_enabled','telegram_enabled','telegram_bot_token','telegram_chat_id','line_system_alerts','line_weekly_report'];
            $settings = [];
        ...
        case 'PUT':
            ...
            foreach (($data['templates'] ?? []) as $k => $tpl) {
                ...
                $clean = [
                    ...
                    'header_size'       => in_array($tpl['header_size'] ?? 'xs', ['xxs', 'xs', 'sm', 'md'], true) ? $tpl['header_size'] : 'xs',
                    'header_weight'     => in_array($tpl['header_weight'] ?? 'bold', ['regular', 'bold'], true) ? $tpl['header_weight'] : 'bold',
                    'header_padding'    => in_array($tpl['header_padding'] ?? 'md', ['none', 'xs', 'sm', 'md', 'lg', 'xl'], true) ? $tpl['header_padding'] : 'md',
                    ...
                    'hero_size'         => in_array($tpl['hero_size'] ?? 'full', ['full', 'xxs', 'xs', 'sm', 'md', 'lg', 'xl', 'xxl', '3xl', '4xl', '5xl'], true) ? $tpl['hero_size'] : 'full',
                    'hero_mode'         => in_array($tpl['hero_mode'] ?? 'cover', ['cover', 'fit'], true) ? $tpl['hero_mode'] : 'cover',
                    ...
                    'body_weight'       => in_array($tpl['body_weight'] ?? 'regular', ['regular', 'bold'], true) ? $tpl['body_weight'] : 'regular',
                    'body_align'        => in_array($tpl['body_align'] ?? 'start', ['start', 'center', 'end'], true) ? $tpl['body_align'] : 'start',
                    'body_wrap'         => (($tpl['body_wrap'] ?? '1') === '1' || ($tpl['body_wrap'] ?? '1') === true) ? '1' : '0',
                    'body_spacing'      => in_array($tpl['body_spacing'] ?? 'none', ['none', 'xs', 'sm', 'md', 'lg', 'xl'], true) ? $tpl['body_spacing'] : 'none',
                    'body_padding'      => in_array($tpl['body_padding'] ?? 'md', ['none', 'xs', 'sm', 'md', 'lg', 'xl'], true) ? $tpl['body_padding'] : 'md',
                    'body_background'   => ($tpl['body_background'] ?? '') !== '' && preg_match('/^#[0-9a-fA-F]{6}$/', (string)$tpl['body_background']) ? (string)$tpl['body_background'] : '',
                    'body_justify'      => in_array($tpl['body_justify'] ?? 'flex-start', ['flex-start', 'center', 'flex-end', 'space-between', 'space-around', 'space-evenly'], true) ? $tpl['body_justify'] : 'flex-start',
                    'body_separator'    => (($tpl['body_separator'] ?? '0') === '1' || ($tpl['body_separator'] ?? '0') === true) ? '1' : '0',
                    ...
                    'bubble_direction'  => in_array($tpl['bubble_direction'] ?? 'ltr', ['ltr', 'rtl'], true) ? $tpl['bubble_direction'] : 'ltr',
                    'bubble_size'       => in_array($tpl['bubble_size'] ?? '', ['', 'nano', 'micro', 'deci', 'hecto', 'kilo', 'mega', 'giga'], true) ? $tpl['bubble_size'] : '',
