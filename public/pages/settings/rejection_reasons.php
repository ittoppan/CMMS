...
<div class="space-y-4">
    <div class="flex items-center justify-between"><div><h1 class="text-2xl font-bold text-primary">สาเหตุการปฏิเสธ</h1><p class="mt-1 text-sm text-muted">Rejection Reasons</p></div><a href="index.php" class="text-sm text-primary-600 hover:text-primary-700">&larr; กลับ</a></div>
    ...
    <div class="card p-4"><h2 class="text-lg font-semibold mb-3"><?=$editRow?'แก้ไข':'เพิ่ม'?>สาเหตุการปฏิเสธ</h2>
        <form method="post" class="grid grid-cols-1 sm:grid-cols-3 gap-3"><?php if($editRow):?><input type="hidden" name="id" value="<?=$editRow['id']?>"><?php endif;?>
            ...
            <div><label class="block text-xs font-medium text-secondary">โมดูล</label><select name="module" class="input input-bordered w-full mt-1"><?php foreach($modules as$m):?><option value="<?=$m?>" <?=$editRow&&$editRow['module']===$m?'selected':''?>><?=$m?></option><?php endforeach;?></select></div>
    ...
    <div class="card overflow-hidden">
        <table class="data-table cmms-stack-table"><thead class="bg-subtle"><tr><th>รหัส</th><th>ชื่อ</th><th>โมดูล</th><th>สถานะ</th><th>จัดการ</th></tr></thead>
