...
<div class="space-y-4">
    <div class="flex items-center justify-between"><div><h1 class="text-2xl font-bold text-gray-900">แท็กงานซ่อม</h1><p class="mt-1 text-sm text-gray-500">Repair Tags</p></div><a href="index.php" class="text-sm text-primary-600 hover:text-primary-700">&larr; กลับ</a></div>
    ...
    <div class="card p-4"><h2 class="text-lg font-semibold mb-3"><?=$editRow?'แก้ไข':'เพิ่ม'?>แท็ก</h2>
        <form method="post" class="grid grid-cols-1 sm:grid-cols-3 gap-3"><?php if($editRow):?><input type="hidden" name="id" value="<?=$editRow['id']?>"><?php endif;?>
            <div><label class="block text-xs font-medium text-gray-600">ชื่อแท็ก</label><input type="text" name="name" value="<?=$editRow?htmlspecialchars($editRow['name']):''?>" required class="input input-bordered w-full mt-1"></div>
            <div><label class="block text-xs font-medium text-gray-600">สี</label><input type="color" name="color" value="<?=$editRow?htmlspecialchars($editRow['color']):'#3B82F6'?>" class="input input-bordered w-full mt-1 h-10 w-20 p-1"></div>
    ...
    <div class="card overflow-hidden">
        <table class="min-w-full divide-y divide-gray-200"><thead class="bg-gray-50"><tr><th>ชื่อแท็ก</th><th>สี</th><th>สถานะ</th><th>จัดการ</th></tr></thead>
            <tbody class="divide-y divide-gray-200"><?php foreach($rows as$r):?><tr class="hover:bg-gray-50"><td class="px-4 py-3 text-sm"><?=htmlspecialchars($r['name'])?></td><td class="px-4 py-3 text-sm"><span class="inline-block w-6 h-6 rounded border" style="background:<?=htmlspecialchars($r['color'])?>"></span> <?=htmlspecialchars($r['color'])?></td><td class="px-4 py-3 text-sm"><span class="badge <?=$r['is_active']?'bg-green-100 text-green-800':'bg-gray-100 text-gray-800'?>"><?=$r['is_active']?'Active':'Inactive'?></span></td><td class="px-4 py-3 text-sm space-x-2"><a href="?edit=<?=$r['id']?>" class="text-primary-600">แก้ไข</a><a href="?delete=<?=$r['id']?>" class="text-red-600" onclick="return confirm('ลบรายการนี้?')">ลบ</a></td></tr><?php endforeach;?><?php if(empty($rows)):?><tr><td colspan="4" class="px-4 py-8 text-center text-gray-500">ไม่มีข้อมูล</td></tr><?php endif;?></tbody>
        </table>
    </div>
</div>
<?php renderFooter(); ?>
