...
?>
<div class="space-y-4">
    <div class="flex items-center justify-between"><div><h1 class="text-2xl font-bold text-primary">จัดการสถานที่</h1><p class="mt-1 text-sm text-muted">Locations (Parent/Child)</p></div><a href="index.php" class="text-sm text-primary-600 hover:text-primary-700">&larr; กลับ</a></div>
    ...
    <div class="card p-4">
        ...
        <form method="post" class="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div><label class="block text-xs font-medium text-secondary">รหัสสถานที่</label><input type="text" name="code" value="<?= $editRow?htmlspecialchars($editRow['code']):'' ?>" required class="input input-bordered w-full mt-1"></div>
            <div><label class="block text-xs font-medium text-secondary">ชื่อสถานที่</label><input type="text" name="name" value="<?= $editRow?htmlspecialchars($editRow['name']):'' ?>" required class="input input-bordered w-full mt-1"></div>
            <div><label class="block text-xs font-medium text-secondary">ประเภท</label><select name="type" class="input input-bordered w-full mt-1"><?php foreach(['building','floor','zone','area','sub_location'] as $t): ?><option value="<?=$t?>" <?= $editRow&&$editRow['type']===$t?'selected':'' ?>><?=$t?></option><?php endforeach; ?></select></div>
            <div><label class="block text-xs font-medium text-secondary">สถานที่หลัก (Parent)</label><select name="parent_id" class="input input-bordered w-full mt-1"><option value="">ไม่มี</option><?php foreach ($parents as $p): ?><option value="<?=$p['id']?>" <?= $editRow&&$editRow['parent_id']==$p['id']?'selected':'' ?>><?=htmlspecialchars($p['code'].' - '.$p['name'])?></option><?php endforeach; ?></select></div>
