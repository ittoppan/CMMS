    ...
    <div class="card overflow-hidden">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50"><td class="px-4 py-3 text-sm font-medium text-gray-900"><?= htmlspecialchars($r['code']) ?></td><td class="px-4 py-3 text-sm text-gray-700"><?= htmlspecialchars($r['name']) ?></td><td class="px-4 py-3 text-sm text-gray-600"><?= htmlspecialchars($r['type']) ?></td><td class="px-4 py-3 text-sm text-gray-600"><?= htmlspecialchars($r['parent_name']??'-') ?></td><td class="px-4 py-3 text-sm"><span class="badge <?= $r['is_active']?'bg-green-100 text-green-800':'bg-gray-100 text-gray-800' ?>"><?= $r['is_active']?'Active':'Inactive' ?></span></td><td class="px-4 py-3 text-sm space-x-2"><a href="?edit=<?= $r['id'] ?>" class="text-primary-600 hover:text-primary-700">แก้ไข</a><a href="?delete=<?= $r['id'] ?>" class="text-red-600 hover:text-red-700" onclick="return confirm('ลบรายการนี้?')">ลบ</a></td></tr>
                <?php endforeach; ?>
