...
$pageTitle = 'รหัสการซ่อม - CMMS-TPT';
renderHeader();
$pdo = getDb(); $msg = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($id) { $pdo->prepare('UPDATE repair_codes SET code=?,name=?,description=?,category=?,is_active=? WHERE id=?')->execute([$code,$name,$desc,$cat,$active,$id]); $msg='updated'; }
        else { $pdo->prepare('INSERT INTO repair_codes (code,name,description,category,is_active) VALUES (?,?,?,?,?)')->execute([$code,$name,$desc,$cat,$active]); $msg='created'; }
    } catch (Exception $e) { $msg='error: '.$e->getMessage(); }
}
$delId=(int)($_GET['delete']??0); if($delId){try{$pdo->prepare('DELETE FROM repair_codes WHERE id=?')->execute([$delId]);$msg='deleted';}catch(Exception$e){$msg='error: '.$e->getMessage();}}
$editId=(int)($_GET['edit']??0);$editRow=null;if($editId){$s=$pdo->prepare('SELECT*FROM repair_codes WHERE id=?');$s->execute([$editId]);$editRow=$s->fetch();}
$rows=$pdo->query('SELECT*FROM repair_codes ORDER BY code')->fetchAll(); ?>
<div class="space-y-4">
    <div class="flex items-center justify-between"><div><h1 class="text-2xl font-bold text-primary">รหัสการซ่อม</h1><p class="mt-1 text-sm text-muted">Repair Codes</p></div><a href="index.php" class="text-sm text-primary-600 hover:text-primary-700">&larr; กลับ</a></div>
    ...
    <div class="card p-4"><h2 class="text-lg font-semibold mb-3"><?=$editRow?'แก้ไข':'เพิ่ม'?>รหัสการซ่อม</h2>
        <form method="post" class="grid grid-cols-1 sm:grid-cols-3 gap-3"><?php if($editRow):?><input type="hidden" name="id" value="<?=$editRow['id']?>"><?php endif;?>
            ...
            <div><label class="block text-xs font-medium text-secondary">ชื่อ</label><input type="text" name="name" value="<?=$editRow?htmlspecialchars($editRow['name']):''?>" required class="input input-bordered w-full mt-1"></div>
            ...
            <div><label class="block text-xs font-medium text-secondary">หมวดหมู่</label><input type="text" name="category" value="<?=$editRow?htmlspecialchars($editRow['category']??''):''?>" class="input input-bordered w-full mt-1"></div>
