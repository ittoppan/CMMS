...
<div class="space-y-4">
    ...
    <div class="card overflow-hidden">
        <table class="min-w-full divide-y divide-gray-200"><thead class="bg-gray-50"><tr><th>โมดูล</th><th>ชื่อกฎ</th><th>เงื่อนไข</th><th>ผู้รับมอบหมาย</th><th>ลำดับ</th><th>สถานะ</th><th>จัดการ</th></tr></thead>
            <tbody class="divide-y divide-gray-200"><?php foreach($rows as$r):?><tr class="hover:bg-gray-50"><td class="px-4 py-3 text-sm"><?=$r['module']?></td><td class="px-4 py-3 text-sm font-medium"><?=htmlspecialchars($r['rule_name'])?></td><td class="px-4 py-3 text-sm"><?=$r['criteria_type']?> #<?=$r['criteria_value']?></td><td class="px-4 py-3 text-sm"><?=htmlspecialchars($r['assignee_name']??'-')?></td><td class="px-4 py-3 text-sm"><?=$r['priority']?></td><td class="px-4 py-3 text-sm"><span class="badge <?=$r['is_active']?'status-active':'status-inactive'?>"><?=$r['is_active']?'Active':'Inactive'?></span></td><td class="px-4 py-3 text-sm space-x-2"><a href="?edit=<?=$r['id']?>" class="text-primary-600">แก้ไข</a><a href="?delete=<?=$r['id']?>" class="text-red-600" onclick="return confirm('ลบรายการนี้?')">ลบ</a></td></tr><?php endforeach;?><?php if(empty($rows)):?><tr><td colspan="7" class="px-4 py-8 text-center text-gray-500">ไม่มีข้อมูล</td></tr><?php endif;?></tbody>
        </table>
    </div>
</div>
<?php renderFooter(); ?>
