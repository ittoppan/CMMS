...
<div class="space-y-4">
    <div class="flex items-center justify-between"><div><h1 class="text-2xl font-bold text-primary">กฎการมอบหมายงานอัตโนมัติ</h1><p class="mt-1 text-sm text-muted">Auto Assignment Rules</p></div><a href="index.php" class="text-sm text-primary-600 hover:text-primary-700">&larr; กลับ</a></div>
    ...
    <div class="card p-4"><h2 class="text-lg font-semibold mb-3"><?=$editRow?'แก้ไข':'เพิ่ม'?>กฎ</h2>
        <form method="post" class="grid grid-cols-1 sm:grid-cols-3 gap-3"><?php if($editRow):?><input type="hidden" name="id" value="<?=$editRow['id']?>"><?php endif;?>
            ...
            <div><label class="block text-xs font-medium text-secondary">ชื่อกฎ</label><input type="text" name="rule_name" value="<?=$editRow?htmlspecialchars($editRow['rule_name']):''?>" required class="input input-bordered w-full mt-1"></div>
            <div><label class="block text-xs font-medium text-secondary">ประเภทเงื่อนไข</label><select name="criteria_type" class="input input-bordered w-full mt-1"><?php foreach(['location','asset_type','asset','checklist_template','plan','department','work_zone'] as$c):?><option value="<?=$c?>" <?=$editRow&&$editRow['criteria_type']===$c?'selected':''?>><?=$c?></option><?php endforeach;?></select></div>
            <div><label class="block text-xs font-medium text-secondary">ค่าเงื่อนไข (ID)</label><input type="number" name="criteria_value" value="<?=$editRow?$editRow['criteria_value']:'0'?>" required class="input input-bordered w-full mt-1"></div>
            <div><label class="block text-xs font-medium text-secondary">ผู้รับมอบหมาย</label><select name="assignee_id" class="input input-bordered w-full mt-1"><?php foreach($users as$u):?><option value="<?=$u['id']?>" <?=$editRow&&$editRow['assignee_id']==$u['id']?'selected':''?>><?=htmlspecialchars($u['full_name'])?></option><?php endforeach;?></select></div>
            <div><label class="block text-xs font-medium text-secondary">ลำดับความสำคัญ</label><input type="number" name="priority" value="<?=$editRow?$editRow['priority']:'0'?>" class="input input-bordered w-full mt-1"></div>
    ...
    <div class="card overflow-hidden">
        <table class="data-table cmms-stack-table"><thead class="bg-subtle"><tr><th>โมดูล</th><th>ชื่อกฎ</th><th>เงื่อนไข</th><th>ผู้รับมอบหมาย</th><th>ลำดับ</th><th>สถานะ</th><th>จัดการ</th></tr></thead>
