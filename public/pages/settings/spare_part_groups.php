...
?>
<div class="space-y-4">
    <div class="flex items-center justify-between"><div><h1 class="text-2xl font-bold text-primary">กลุ่มอะไหล่</h1><p class="mt-1 text-sm text-muted">Spare Part Groups</p></div><a href="index.php" class="text-sm text-primary-600 hover:text-primary-700">&larr; กลับ</a></div>
    ...
    <div class="card p-4"><h2 class="text-lg font-semibold mb-3"><?=$editRow?'แก้ไข':'เพิ่ม'?>กลุ่มอะไหล่</h2>
        <form method="post" class="grid grid-cols-1 sm:grid-cols-3 gap-3"><?php if($editRow):?><input type="hidden" name="id" value="<?=$editRow['id']?>"><?php endif;?>
            <div><label class="block text-xs font-medium text-secondary">รหัสกลุ่ม</label><input type="text" name="code" value="<?=$editRow?htmlspecialchars($editRow['code']):''?>" required class="input input-bordered w-full mt-1"></div>
            <div><label class="block text-xs font-medium text-secondary">ชื่อกลุ่ม</label><input type="text" name="name" value="<?=$editRow?htmlspecialchars($editRow['name']):''?>" required class="input input-bordered w-full mt-1"></div>
    ...
    <div class="card p-4">
        ...
        <table class="data-table cmms-stack-table"><thead class="bg-subtle"><tr><th>รหัสอะไหล่</th><th>ชื่อ</th><th>จัดการ</th></tr></thead>
            <tbody class="divide-y divide-line"><?php foreach($items as$i):?><tr class="hover:bg-subtle"><td data-label="รหัสอะไหล่" class="px-4 py-2 text-sm"><?=htmlspecialchars($i['code'])?></td><td data-label="ชื่อ" class="px-4 py-2 text-sm"><?=htmlspecialchars($i['name'])?></td><td data-label="จัดการ" class="px-4 py-2 text-sm"><form method="post" class="inline"><input type="hidden" name="del_item" value="1"><input type="hidden" name="gid" value="<?=$gid?>"><input type="hidden" name="spare_part_id" value="<?=$i['id']?>"><button type="submit" class="text-red-600 text-sm" onclick="return confirm('ลบรายการนี้?')">ลบ</button></form></td></tr><?php endforeach;?><?php if(empty($items)):?><tr><td colspan="3" class="cmms-empty-state-cell">ยังไม่มีอะไหล่ในกลุ่มนี้</td></tr><?php endif;?></tbody>
        </table>
    </div>
    <?php endif; endif; ?>
</div>
