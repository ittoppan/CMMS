...
<div class="space-y-4">
    ...
    <div class="card p-4"><h2 class="text-lg font-semibold mb-3"><?=$editRow?'แก้ไข':'เพิ่ม'?>การแจ้งเตือน</h2>
        <form method="post" class="grid grid-cols-1 sm:grid-cols-2 gap-3"><?php if($editRow):?><input type="hidden" name="id" value="<?=$editRow['id']?>"><?php endif;?>
            ...
            <div><label class="block text-xs font-medium text-gray-600">เหตุการณ์</label><input type="text" name="event" value="<?=$editRow?htmlspecialchars($editRow['event']):''?>" required class="input input-bordered w-full mt-1" placeholder="e.g. repair_created"></div>
            <div><label class="block text-xs font-medium text-gray-600">หัวข้ออีเมล</label><input type="text" name="subject" value="<?=$editRow?htmlspecialchars($editRow['subject']??''):''?>" class="input input-bordered w-full mt-1"></div>
            <div><label class="block text-xs font-medium text-gray-600">ผู้รับ (comma-separated)</label><input type="text" name="recipients" value="<?=$editRow?htmlspecialchars($editRow['recipients']??''):''?>" class="input input-bordered w-full mt-1" placeholder="email หรือ role ID"></div>
            <div class="sm:col-span-2"><label class="block text-xs font-medium text-gray-600">เนื้อหา (Template)</label><textarea name="template_body" rows="4" class="input input-bordered w-full mt-1 w-full"><?=$editRow?htmlspecialchars($editRow['template_body']??''):''?></textarea></div>
    ...
    <div class="card overflow-hidden">
        <table class="min-w-full divide-y divide-gray-200"><thead class="bg-gray-50"><tr><th>โมดูล</th><th>เหตุการณ์</th><th>หัวข้อ</th><th>ผู้รับ</th><th>สถานะ</th><th>จัดการ</th></tr></thead>
            <tbody class="divide-y divide-gray-200"><?php foreach($rows as$r):?><tr class="hover:bg-gray-50"><td class="px-4 py-3 text-sm"><?=$r['module']?></td><td class="px-4 py-3 text-sm"><code class="bg-gray-100 px-1 rounded text-xs"><?=htmlspecialchars($r['event'])?></code></td><td class="px-4 py-3 text-sm max-w-xs truncate"><?=htmlspecialchars($r['subject']??'-')?></td><td class="px-4 py-3 text-sm max-w-xs truncate"><?=htmlspecialchars($r['recipients']??'-')?></td><td class="px-4 py-3 text-sm"><span class="badge <?=$r['is_active']?'bg-green-100 text-green-800':'bg-gray-100 text-gray-800'?>"><?=$r['is_active']?'Active':'Inactive'?></span></td><td class="px-4 py-3 text-sm space-x-2"><a href="?edit=<?=$r['id']?>" class="text-primary-600">แก้ไข</a><a href="?delete=<?=$r['id']?>" class="text-red-600" onclick="return confirm('ลบรายการนี้?')">ลบ</a></td></tr><?php endforeach;?><?php if(empty($rows)):?><tr><td colspan="6" class="px-4 py-8 text-center text-gray-500">ไม่มีข้อมูล</td></tr><?php endif;?></tbody>
        </table>
    </div>
</div>
<?php renderFooter(); ?>
