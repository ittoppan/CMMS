...
<div class="space-y-4">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-primary">ประวัติงานซ่อม</h1>
            <p class="mt-1 text-sm text-muted">ดูประวัติงานซ่อมทั้งหมด พร้อมผลการประเมิน</p>
        </div>
        <div class="flex gap-2">
            ...
            <span class="inline-flex items-center px-4 py-2 bg-gray-300 text-secondary text-sm font-medium rounded-md cursor-not-allowed">ส่งออก (เร็วๆ นี้)</span>
        </div>
    </div>

    <div class="card p-4 shadow">
        <form method="GET" class="flex flex-wrap items-end gap-3">
            <div><label class="block text-xs font-medium text-secondary mb-1">ค้นหา</label><input type="text" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="ค้นหา..." class="w-48 px-3 py-2 border border-line rounded-md text-sm"></div>
            <div><label class="block text-xs font-medium text-secondary mb-1">สถานะ</label>
                <select name="status" class="px-3 py-2 border border-line rounded-md text-sm">
            ...
            <div><label class="block text-xs font-medium text-secondary mb-1">ความสำคัญ</label>
                <select name="priority" class="px-3 py-2 border border-line rounded-md text-sm">
                    <option value="">ทั้งหมด</option>
            ...
            <div><label class="block text-xs font-medium text-secondary mb-1">ช่าง</label>
                <select name="assigned_to" class="px-3 py-2 border border-line rounded-md text-sm">
                    <option value="">ทั้งหมด</option>
            ...
            <div><label class="block text-xs font-medium text-secondary mb-1">แผนก</label>
                <select name="department_id" class="px-3 py-2 border border-line rounded-md text-sm">
                    <option value="">ทั้งหมด</option>
            ...
            <div><label class="block text-xs font-medium text-secondary mb-1">จากวันที่</label><input type="date" name="date_from" value="<?= htmlspecialchars($filterDateFrom) ?>" class="px-3 py-2 border border-line rounded-md text-sm"></div>
            <div><label class="block text-xs font-medium text-secondary mb-1">ถึงวันที่</label><input type="date" name="date_to" value="<?= htmlspecialchars($filterDateTo) ?>" class="px-3 py-2 border border-line rounded-md text-sm"></div>
            <div><label class="block text-xs font-medium text-secondary mb-1">Safety</label>
                <select name="safety_related" class="px-3 py-2 border border-line rounded-md text-sm">
                    <option value="">ทั้งหมด</option>
                    <option value="yes" <?= $filterSafety==='yes'?'selected':'' ?>>เฉพาะ Safety</option>
                </select>
            </div>
            <div><label class="block text-xs font-medium text-secondary mb-1">ประเมินผล</label>
                <select name="has_rating" class="px-3 py-2 border border-line rounded-md text-sm">
                    <option value="">ทั้งหมด</option>
            ...
            <a href="history.php" class="px-4 py-2 text-sm text-secondary hover:text-primary underline">ล้างตัวกรอง</a>
            <?php endif; ?>
        </form>
    </div>
    ...
    <div class="card shadow overflow-hidden overflow-x-auto">
        <table class="data-table cmms-stack-table">
            <thead class="bg-subtle">
                <tr>
                    <th class="px-3 py-3 text-left text-xs font-medium text-muted uppercase">หัวข้อ</th>
                    <th class="px-3 py-3 text-left text-xs font-medium text-muted uppercase">ทรัพย์สิน</th>
                    <th class="px-3 py-3 text-left text-xs font-medium text-muted uppercase">ช่าง</th>
                    <th class="px-3 py-3 text-left text-xs font-medium text-muted uppercase">สถานะ</th>
                    <th class="px-3 py-3 text-left text-xs font-medium text-muted uppercase">สำคัญ</th>
                    <th class="px-3 py-3 text-left text-xs font-medium text-muted uppercase">Safety</th>
                    <th class="px-3 py-3 text-left text-xs font-medium text-muted uppercase">คะแนน</th>
                    <th class="px-3 py-3 text-left text-xs font-medium text-muted uppercase">เวลา</th>
                    <th class="px-3 py-3 text-left text-xs font-medium text-muted uppercase">วันที่</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-line">
                ...
                <tr class="hover:bg-subtle">
                    ...
                    <td data-label="ทรัพย์สิน" class="px-3 py-3 text-sm text-secondary"><?= htmlspecialchars(($r['asset_code']??'').' - '.($r['asset_name']??'-')) ?></td>
                    <td data-label="ช่าง" class="px-3 py-3 text-sm text-secondary"><?= htmlspecialchars($r['assigned_name']??'-') ?></td>
                    ...
                    <td data-label="เวลา" class="px-3 py-3 text-xs text-muted">
                    ...
                    <td data-label="วันที่" class="px-3 py-3 text-sm text-muted"><?= htmlspecialchars(date('d/m/Y', strtotime($r['created_at']))) ?></td>
                </tr>
                <?php endforeach; ?>
                <?php endif; ?>
            </tbody>
